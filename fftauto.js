...
const User = [
	...
	},
	{
		type:'input',
		name:'target',
		message:'Lebokno target ig ne (ra sah nganggo @[at])',
		validate: function(value){
			if(!value) return 'Can\'t Empty';
			return true;
		}
	},
	{
		type:'input',
		name:'text',
		message:'Dekekno kata-katane cok (Gunakan Pemisah [|] bila lebih dari 1)',
		validate: function(value){
			if(!value) return 'Can\'t Empty';
			return true;
		}
	},
	{
		type:'input',
		name:'sleep',
		message:'Jeda ne piro (In MiliSeconds)',
		validate: function(value){
...
const Excute = async function(User, TargetUsername, Text, Sleep){
	try {
		console.log(chalk`{yellow \n | Sik lagi nyoba login sabar yo .....}`)
		const doLogin = await Login(User);
		console.log(chalk`{green  | Oke sip, Sukses, try to get Followers Target ....}`)
			...
			for (let i = 0; i < TargetResult.length; i++) {
				await Promise.all(TargetResult[i].map(async(akun) => {
					if (!getFollowers.includes(akun.id) && akun.params.isPrivate === false) {
						...
						console.log(chalk`{bold.yellow [SKIPPED]}${akun.params.username} => WONG ALAY AKUN E DI PRIVATE`)
					}
				}));
				console.log(chalk`{yellow Sabar nunggu ${Sleep} MiliSeconds}`);
				await delay(Sleep);
			}
...
console.log(chalk`
{bold Ojo lali doa ben do follback awakmu}
{green Adi Ganteng ga ada duanya}
{bold.red Ojo maruk-maruk mengko akunmu disable}
`);
